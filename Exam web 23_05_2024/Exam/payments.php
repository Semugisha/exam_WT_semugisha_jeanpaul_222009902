...
<body style="background-image: url('./Images/P.jpg'); background-repeat: no-repeat; background-size: cover;">
  <header>
    <h1>Payments</h1>
  </header>
...
<li style="display: inline; margin-right: 10px;"><a href="./subscriptions.php">Subscriptions</a>
  ...
  <section>
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $stmt = $connection->prepare("INSERT INTO payments (id, profile_id, amount, payment_date, status) VALUES (?, ?, ?, ?, ?)");
        $stmt->bind_param("issss", $id, $profile_id, $amount, $payment_date, $status);

        // Set parameters from POST data with validation (optional)
        $id = intval($_POST['id']); // Ensure integer for id
        $profile_id = htmlspecialchars($_POST['profile_id']); // Prevent XSS
        $amount = htmlspecialchars($_POST['amount']); // Prevent XSS
        $payment_date = htmlspecialchars($_POST['payment_date']); // Prevent XSS
      ...
      if ($result->num_rows > 0) {
          ...
          while ($row = $result->fetch_assoc()) {
              ...
              echo "<tr>
                  ...
                  <td>" . $row['status'] . "</td>
                  <td><a style='padding:4px' href='delete_payments.php?id=$id'>Delete</a></td>
                  <td><a style='padding:4px' href='update_payments.php?id=$id'>Update</a></td>
              </tr>";
          }
      } else {
